/**
 * Check all transaction entries for August to see what's missing
 */
...
async function checkAllAugustTransactions() {
  try {
    console.log('🔍 Checking ALL transaction entries for August 2025...');
    
    // Define August 2025 date range
    const augustStart = new Date('2025-08-01');
    const augustEnd = new Date('2025-08-31');
    
    // Get ALL transaction entries for August 2025 (no source filter)
    ...
    // Check if there are any transaction entries with dates outside August but should be included
    console.log(`\n🔍 Checking for transaction entries with dates outside August range...`);
    
    const entriesOutsideAugust = await TransactionEntry.find({
      date: { $lt: augustStart }
    }).sort({ date: -1 }).limit(10);
    
    if (entriesOutsideAugust.length > 0) {
      console.log(`   Found ${entriesOutsideAugust.length} recent entries before August:`);
      entriesOutsideAugust.forEach(entry => {
        console.log(`      - ${entry.transactionId}: $${entry.totalDebit || 0} - ${entry.date.toISOString().split('T')[0]} - ${entry.source}`);
      });
    }
    
  } catch (error) {
    console.error('❌ Error checking August transactions:', error);
  } finally {
