...
class RentalAccrualService {
    ...
    static async processLeaseStart(application) {
        try {
            ...
            // 🆕 SKIP AUTO-INVOICE: Invoice creation is now handled by the cron service
            // This prevents timeout issues and ensures consistent invoice processing
            console.log(`⏭️ Skipping invoice creation - handled by invoice cron service`);
    ...
    /**
     * 🆕 SIMPLIFIED: Create lease start accruals only (for manual student creation)
     * This creates the essential accounting entries without heavy operations like invoices and auto-backfill
     */
    static async createLeaseStartAccrualsOnly(application) {
        try {
            console.log(`🏠 Creating lease start accruals only for ${application.firstName} ${application.lastName}`);
            ...
            if (existingEntries) {
                console.log(`⚠️ Lease start entries already exist for this application ${application.applicationCode}`);
                return { 
                    success: true, 
                    message: 'Lease start entries already exist',
                    existingTransaction: existingEntries._id
                };
            }
            ...
            console.log(`📅 Prorated rent computed: $${proratedRent.toFixed(2)}`);
            ...
            const transactionEntry = new TransactionEntry({
                ...
                date: startDate,
                description: `Lease start accounting entries: ${application.firstName} ${application.lastName}`,
                reference: application.student.toString(),
                entries,
                totalDebit,
                totalCredit,
                source: 'rental_accrual',
                sourceId: application.student,
                sourceModel: 'Lease',
                residence: application.residence,
                createdBy: '68b7909295210ad2fa2c5dcf', // System user ID
                status: 'posted',
                metadata: {
                    ...
                    type: 'lease_start',
                    leaseStartDate: application.startDate,
                    accrualMonth: startDate.getMonth() + 1,
                    accrualYear: startDate.getFullYear(),
                    proratedRent,
                    adminFee,
                    securityDeposit,
                    totalDebit,
            ...
            // Log transaction creation
            await logSystemOperation('create', 'TransactionEntry', transactionEntry._id, {
                source: 'Rental Accrual Service (Manual)',
                type: 'lease_start',
                applicationId: application._id,
                studentId: application.student,
                studentName: `${application.firstName} ${application.lastName}`,
            ...
            console.log(`✅ Lease start accruals created for ${application.firstName} ${application.lastName}`);
            console.log(`   Total Debit: $${totalDebit.toFixed(2)}`);
            console.log(`   Total Credit: $${totalCredit.toFixed(2)}`);
            
            return {
                success: true,
                message: 'Lease start accruals created successfully',
                transactionId: transaction.transactionId,
            ...
            console.error('Error creating lease start accruals:', error);
            return { success: false, error: error.message };
        }
    }
