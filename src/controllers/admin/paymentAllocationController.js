...
const getStudentsWithOutstandingBalances = async (req, res) => {
    try {
        ...
        console.log(`👥 Getting students with outstanding balances`);
        ...
        arTransactions.forEach(transaction => {
            ...
            if (arEntry) {
                const studentId = arEntry.accountCode.split('-')[4]; // Extract student ID from account code
                const monthKey = arEntry.accountCode.split('-').slice(1, 3).join('-');
                ...
                // Calculate remaining balance (original debit minus any credits)
                let remainingBalance = arEntry.debit;
                
                // Subtract any payment allocations for this month
                transaction.metadata?.paymentAllocations?.forEach(allocation => {
                    remainingBalance -= allocation.amount;
                });

                if (remainingBalance > 0) {
                    studentBalances[studentId].monthlyBalances[monthKey] = {
                        month: monthKey,
                        balance: remainingBalance,
                        originalDebt: arEntry.debit,
                        transactionId: transaction._id
                    };

                    studentBalances[studentId].totalBalance += remainingBalance;

                    // Track oldest and newest balances
                    if (!studentBalances[studentId].oldestBalance || monthKey < studentBalances[studentId].oldestBalance) {
                        studentBalances[studentId].oldestBalance = monthKey;
                    }
                    if (!studentBalances[studentId].newestBalance || monthKey > studentBalances[studentId].newestBalance) {
                        studentBalances[studentId].newestBalance = monthKey;
                    }
                }
            }
        });

        // Convert to array and filter students with balances
        let studentsArray = Object.values(studentBalances)
            .filter(student => student.totalBalance > 0)
            .map(student => ({
                ...student,
                monthlyBalances: Object.values(student.monthlyBalances).sort((a, b) => a.month.localeCompare(b.month)),
                monthsWithBalance: Object.keys(student.monthlyBalances).length
            }));
        ...
        // Apply limit
        studentsArray = studentsArray.slice(0, parseInt(limit));

        // Get student details for the results
        const studentIds = studentsArray.map(s => s.studentId);
        const students = await User.find({ _id: { $in: studentIds } }, 'firstName lastName email phone');

        // Merge student details
        const studentsWithDetails = studentsArray.map(student => {
