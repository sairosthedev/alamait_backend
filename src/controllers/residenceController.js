...
// Get 1ACP residence
exports.getOneACPResidence = async (req, res) => {
    try {
...
// Public: Get status and occupancy for a specific room in a residence by name and room number
exports.getRoomStatusByResidenceAndRoomNumber = async (req, res) => {
    try {
        const residence = await Residence.findOne({ name: req.params.name });
        if (!residence) {
            return res.status(404).json({
        ...
        const room = residence.rooms.find(r => r.roomNumber === req.params.roomNumber);
        if (!room) {
            return res.status(404).json({
                success: false,
                message: 'Room not found'
