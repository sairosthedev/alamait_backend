...
async function restoreCindyData() {
  try {
    ...
    console.log('🔄 RESTORING CINDY GWEKWERERE DATA FROM JSON FILE\n');
    ...
    cindyTransactions.forEach((tx, index) => {
      console.log(`   ${index + 1}. ${tx.description} (${tx.date})`);
      console.log(`      Source: ${tx.source}, Amount: $${tx.totalDebit}`);
    });

    // 4. Restore the transactions
    console.log('\n📊 STEP 4: Restoring Transactions\n');
