...
/**
 * COMPLETE RENTAL ACCRUAL SYSTEM FIX - VERSION 2
 * 
 * This script will:
 ...
 * 5. Follow proper Transaction model schema requirements
 */

async function fixRentalAccrualSystemV2() {
  try {
    console.log('\n🔧 FIXING RENTAL ACCRUAL SYSTEM - VERSION 2');
    console.log('=============================================\n');
    ...
    // ========================================
    // STEP 2: GET ACTIVE DEBTORS AND RESIDENCES
    // ========================================
    console.log('\n🔍 STEP 2: Getting Active Debtors and Residences\n');
    ...
    const residenceIds = [...new Set(activeDebtors.map(d => d.residence).filter(Boolean))];
    ...
    activeDebtors.forEach((debtor, index) => {
      const residenceName = debtor.residence ? (residenceMap[debtor.residence.toString()]?.name || 'Unknown') : 'N/A';
      console.log(`   ${index + 1}. ${debtor.debtorCode} - Room ${debtor.roomNumber || 'N/A'} - Residence: ${residenceName} - Balance: $${debtor.currentBalance || 0}`);
    });
    ...
    for (const monthInfo of months) {
      ...
      for (const debtor of activeDebtors) {
        try {
          ...
          // Get residence info (required by Transaction model)
          const residence = debtor.residence;
          if (!residence) {
            console.log(`   ⚠️  ${debtor.debtorCode}: No residence assigned, skipping`);
            continue;
          }
          
          // Create transaction entry directly (simpler approach, no Transaction model)
          const entries = [
          ...
          // Create transaction entry directly
