...
/**
 * COMPLETE RENTAL ACCRUAL SYSTEM - FIXED VERSION
 * 
 * This script will:
 * 1. Create monthly rent accruals for every debtor from their billing period dates
 * 2. Create proper double-entry accounting entries
 * 3. Link all accruals to specific debtors using correct sourceModel
 * 4. Handle admin fees and security deposits properly
 * 5. Ensure proper accrual vs cash basis accounting
 */

async function createCompleteRentalAccrualsFixed() {
  try {
    console.log('\n🏠 CREATING COMPLETE RENTAL ACCRUAL SYSTEM - FIXED');
    console.log('====================================================\n');
    ...
    // ========================================
    // STEP 2: GET ALL DEBTORS WITH BILLING PERIODS
    // ========================================
    console.log('\n🔍 STEP 2: Getting All Debtors with Billing Period Information\n');
    ...
    for (const debtor of allDebtors) {
      try {
        ...
        // Get billing period dates from debtor
        let startDate, endDate;
        
        if (debtor.billingPeriod && debtor.billingPeriod.startDate && debtor.billingPeriod.endDate) {
          startDate = new Date(debtor.billingPeriod.startDate);
          endDate = new Date(debtor.billingPeriod.endDate);
          console.log(`   Billing Period: ${startDate.toDateString()} to ${endDate.toDateString()}`);
        } else if (debtor.startDate && debtor.endDate) {
          startDate = new Date(debtor.startDate);
          endDate = new Date(debtor.endDate);
          console.log(`   Lease Period: ${startDate.toDateString()} to ${endDate.toDateString()}`);
        } else {
          // Default to 2025 if no dates set
          startDate = new Date('2025-01-01');
          endDate = new Date('2025-12-31');
          console.log(`   Default Period: ${startDate.toDateString()} to ${endDate.toDateString()}`);
        }
        ...
        for (const monthInfo of months) {
          try {
            ...
            // Create transaction entry with CORRECT sourceModel
            const transactionEntry = new TransactionEntry({
              ...
              source: 'rental_accrual',
              sourceId: debtor._id,
              sourceModel: 'Lease', // ✅ FIXED: Using 'Lease' instead of 'Debtor'
...
createCompleteRentalAccrualsFixed();
