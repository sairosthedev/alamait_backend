...
async function restoreAllCindyData() {
  try {
    ...
    console.log('🔄 RESTORING ALL CINDY GWEKWERERE DATA (INCLUDING PAYMENTS)\n');
    ...
    // 2. Filter for ALL Cindy Gwekwerere related transactions
    console.log('\n📊 STEP 2: Filtering Cindy Gwekwerere Related Transactions\n');
    
    const cindyRelatedTransactions = transactions.filter(tx => {
      // Check for Cindy Gwekwerere in description
      const hasCindyInDescription = tx.description && tx.description.includes('Cindy Gwekwerere');
      
      // Check for Cindy's student ID in metadata
      const hasCindyStudentId = tx.metadata && tx.metadata.studentId === '68aeaf7a8d70befd6ad29b18';
      
      // Check for Cindy's student ID in account codes
      const hasCindyAccountCode = tx.entries && tx.entries.some(entry => 
        entry.accountCode && entry.accountCode.includes('68aeaf7a8d70befd6ad29b18')
      );
      
      return hasCindyInDescription || hasCindyStudentId || hasCindyAccountCode;
    });

    console.log(`📋 Found ${cindyRelatedTransactions.length} Cindy Gwekwerere related transactions in JSON`);

    if (cindyRelatedTransactions.length === 0) {
    ...
    cindyRelatedTransactions.forEach((tx, index) => {
      console.log(`   ${index + 1}. ${tx.description}`);
      console.log(`      Source: ${tx.source}, Amount: $${tx.totalDebit}`);
      console.log(`      Student ID: ${tx.metadata?.studentId || 'Not set'}`);
    ...
    let restoredCount = 0;
    for (const txData of cindyRelatedTransactions) {
      try {
    ...
    const restoredCindyTransactions = await TransactionEntry.find({
      $or: [
        ...
        { 'metadata.studentId': '68aeaf7a8d70befd6ad29b18' },
        { 'entries.accountCode': { $regex: /68aeaf7a8d70befd6ad29b18/ } }
      ],
      status: 'posted'
    }).sort({ date: 1 });
    ...
    // 8. Show Cindy's specific AR balance
    console.log('\n📊 STEP 8: Cindy Gwekwerere AR Balance\n');
    ...
    console.log('\n🎯 ALL CINDY GWEKWERERE DATA RESTORATION COMPLETE!');
    console.log('   ✅ Restored Cindy Gwekwerere transactions from JSON file');
    console.log('   ✅ Restored Cindy Gwekwerere payment transactions');
