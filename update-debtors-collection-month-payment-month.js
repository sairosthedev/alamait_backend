...
async function updateDebtorsCollection() {
    try {
        ...
        for (const debtor of debtors) {
            try {
                ...
                // Update paymentHistory to include payment month information
                if (debtor.paymentHistory && debtor.paymentHistory.length > 0) {
                    console.log(`   💰 Updating ${debtor.paymentHistory.length} payment history records...`);
                ...
                if ((!debtor.monthlyPayments || debtor.monthlyPayments.length === 0) && 
                    ...
                    debtor.monthlyPayments.forEach(mp => {
                        ...
                        if (mp.paidAmount > 0) {
                            mp.status = mp.paidAmount >= mp.expectedAmount ? 'paid' : 'partial';
                        }
                    });
...
console.log('🔄 Starting Debtors Collection Update...');
updateDebtorsCollection();
